...
/**You should use this struct via [`AsanaClient::remove_followers`].

On request success, this will return a [`RemoveFollowersResponse`].*/
...
impl<'a> ::std::future::IntoFuture for FluentRequest<'a, RemoveFollowersRequest> {
    type Output = httpclient::InMemoryResult<RemoveFollowersResponse>;
    type IntoFuture = ::futures::future::BoxFuture<'a, Self::Output>;
    fn into_future(self) -> Self::IntoFuture {
        Box::pin(async move {
            let url = &format!(
                "/goals/{goal_gid}/removeFollowers", goal_gid = self.params.goal_gid
            );
