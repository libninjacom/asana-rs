...
/**You should use this struct via [`AsanaClient::update_goal_metric`].
...
impl<'a> ::std::future::IntoFuture for FluentRequest<'a, UpdateGoalMetricRequest> {
    type Output = httpclient::InMemoryResult<UpdateGoalMetricResponse>;
    type IntoFuture = ::futures::future::BoxFuture<'a, Self::Output>;
    fn into_future(self) -> Self::IntoFuture {
        Box::pin(async move {
            let url = &format!(
                "/goals/{goal_gid}/setMetricCurrentValue", goal_gid = self.params
                .goal_gid
            );
