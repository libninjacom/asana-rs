...
/**You should use this struct via [`AsanaClient::update_project`].

On request success, this will return a [`UpdateProjectResponse`].*/
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UpdateProjectRequest {
    pub data: Option<ProjectUpdateRequest>,
...
impl FluentRequest<'_, UpdateProjectRequest> {
    pub fn data(mut self, data: ProjectUpdateRequest) -> Self {
...
impl<'a> ::std::future::IntoFuture for FluentRequest<'a, UpdateProjectRequest> {
    type Output = httpclient::InMemoryResult<UpdateProjectResponse>;
    type IntoFuture = ::futures::future::BoxFuture<'a, Self::Output>;
    fn into_future(self) -> Self::IntoFuture {
        Box::pin(async move {
            let url = &format!(
                "/projects/{project_gid}", project_gid = self.params.project_gid
            );
